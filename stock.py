...
class Corperate_Announcements:
    ...
    def _nsefetch(self, url):
        """
        Fetches data using the persistent session.
        """
        encoded_url = urllib.parse.quote(url, safe=":/?&=")
        print("encoded url",encoded_url)
        try:
            self.session.get("https://www.nseindia.com", headers=self.headers, timeout=10)
            r = self.session.get(encoded_url, headers=self.headers, timeout=15)
            r.raise_for_status()
    ...
    def corporate_voting_results(self, **kwargs):
        """
        Fetches and processes corporate voting results data from the NSE API.

        Args:
            data (dict): The JSON data returned from the NSE API.

        Returns:
            pd.DataFrame: A DataFrame containing the processed voting results data.
        """
    ...
    def get_intraday_chart(self, symbol):
        ...
        if "grapthData" in data:
            ...
            for point in chart_data:
                ts = pd.to_datetime(point[0], unit="ms")
                price = point[1]
